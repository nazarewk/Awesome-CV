...
%---------------------------------------------------------
    ...
    {building up homelab} % Organization
    {} % Location
    {Jan 2025 - Feb. 2025} % Date(s)
    {
        \begin{cvitems} % Description(s) of tasks/responsibilities
            \item {Refined a selectively-persistent NixOS setup and onboarded new and existing machines onto it,}
            \item {Built up NixOS-backed dual-stack router configuration with VLANs,}
            \item {Used Mac Mini to run MacOS VMs and Nix builders for arm64,}
            \item {Got a minimal Home Assistant setup running on NixOS on Raspberry Pi 4 with a few HATs,}
            \item {Attempted to integrate Talos Kubernetes into the new network,}
        \end{cvitems}
    }
